...
app.post('/webhook/orders/create', async (req, res) => {
    try {
        ...
        if (shouldSendEmail(warehouseType, supplier, shippingCountry)) {
            emailSent = await sendAutomatedEmail(shippingDetails, productDetails);
...
const sendAutomatedEmail = async (shippingDetails, productDetails) => {
      ...
      <html>
        <body>
          ...
          <ul>
            ...
            <li><strong>Max Approval Amount:</strong> ${productDetails.poNumber}</li>
          </ul>
    ...
    `;
        const info = await transporter.sendMail({
            ...
            to: ["abdullah@behope.ca", "haroon@behope.ca", "orders@behope.ca"],
            subject: `Order Request for Account #62317 - PO ${productDetails.poNumber}`,
            html: emailHtml,
        });
...
async function updateOrderTags(orderId, newTag) {
    try {
        ...
        if (!tagsArray.includes(newTag)) {
            ...
            console.log(`:label: Successfully added tag "${newTag}" to order ${orderId}`);
            return true;
        } else {
